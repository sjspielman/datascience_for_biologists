...
```{r}
# assumes broom library is loaded! it comes installed with tidyverse, but you need to load with library(broom) in the setup chunk.
...
For making QQ plots, we'll use the base R functions `qqnorm()` and `qqline()` (in this case base R is a lot easier than ggplot2). They are run as *separate commands entirely*:
...
This is pretty good, but could be better - the right tail end of the plot has a bit too much deviation, but not so much that I'd worry. So, I'm satisified with the assumptions being met for this model.
...
ggplot(sparrows, aes(x = Age, y = Weight, color = Age)) + 
  ...
  stat_summary(geom = "point",  ## draw a point
               fun.y = "mean",  ## take the mean along the y-axis
               color = "black", size = 4) ## point styling
```

As you can see, those means look about identical!! It makes sense that the model did *not* show evidence that Age affects Weights.
...
This visualization shows us what the model did in numbers: See how the confidence bands for Adult and Young are completely overlapping???? This gives us a strong sense that Age does NOT have an effect on weight here - otherwise, the two trend lines would be fully distinct. Instead, since only skull length was significant, a single trend line is more accurate to represent our conclusions:
