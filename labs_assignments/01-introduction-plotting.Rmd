...
author: "Data Science for Biologists, Spring 2020"
#date: "12/28/2019"
output: 
  html_document:
    highlight: tango
...
This lab focuses on making figures using the `ggplot2` package, which is one of several [`tidyverse`](https://tidyverse.tidyverse.org/) packages. To obtain `ggplot2`, you will need to first _install_ the library, and then _load_ the library for use in your current R session. 

We will, in fact, install the whole suite of `tidyverse` packages which will include `ggplot2`. **You only need to run package installation ONCE within a given R project. Once it has been installed, you can use the package by loading it when needed**.

The command to install packages is `install.packages()` - you must provide package names _in quotes!_ to this function:
```{r, eval=FALSE}
install.packages("tidyverse")
```
This command will take several minutes to run and complete, and you may see red text going down the console - don't worry! These are just messages indicating the stage of installation; they are not errors.
...
Today, we will focus on the first three components (reading from the bottom-up!):

1. **Data** - the actual data that will be used to create the figure
2. **Aesthetics** - how will aspects of the data be _mapped_ into the figure? These include mapped visual components like color, shape, size, etc.  
3. **Geometrics (geoms)** - what will the literal _geometric shape_ be that represents the data? This mostly corresponds to the type of plot you will make, e.g. the geom in a boxplot is a boxplot. The geom in a histogram is a histogram. The geom of a scatterplot is _points_. 
...
Finally, let's tweak the axis labels. By default, the name of the column plotted is used on its axis, and histogram Y-axes are given the name "count". The fastest way (but as we will see, NOT the only way!) to modify this is with the functions `xlab()` and `ylab()` (lab for "label"). Within the Grammar of Graphics, axis titles are regarded as _scales_. 
...
ggplot(iris, aes(x = Sepal.Length)) + 
  geom_histogram(color = "black", fill = "dodgerblue") +
  theme_classic() + 
  xlab("Sepal Length (cm)") + ylab("Count") -> p  ## Assignment!
...
1. A _color_ for the histogram outline of your choosing
2. A _fill_ for the histogram bars of your choosing
3. A _bidwidth_ that gives a meaningful representation of the data. Try at least 3-4 binning options to find the best one.
4. Clean axis labels
5. _A different [theme](https://ggplot2.tidyverse.org/reference/ggtheme.html) from `theme_gray()` or `theme_classic()` - choose one that works for you!
  + **For an added challenge**, install and load the package [`ggthemes`](https://yutannihilation.github.io/allYourFigureAreBelongToUs/ggthemes/) find your favorite
6. A meaningful title. You can add plot titles with the function `ggtitle()`
7. Save your plot as a *png* file with a _reasonable aspect ratio!!_. You'll definitely need to look up `ggsave()` help for getting this done.
...
1. Examine this code, which has an error so it does NOT produce a figure, but shares a reasonably helpful error message. Figure out why and fix the problem by playing around with the code. Hint: What is an aesthetic? What is NOT an aesthetic?
    ```{r, message=FALSE, error=TRUE}
    ggplot(iris, aes(x = Sepal.Length)) + 
      geom_density(fill = Species, alpha = 0.6) 
    ```

2. Examine this code and the resulting figure. You will notice the output has some unexpected components. Figure out why and fix the problem by playing around with the code.
    ```{r, message=FALSE}
    ggplot(iris, aes(x = Sepal.Length)) + 
      geom_density(aes(fill = Species, alpha = 0.6)) 
    ```
...
4. Modify your density plot to show the legend on the _bottom_ of the plot. To achieve this, you will need to modify the theme with the added function/argument: `theme(theme_position = "bottom")` (more/way too many details [here](https://ggplot2.tidyverse.org/reference/theme.html)). 
    + Once you have completed this, make a second plot where the lelgend is on the TOP of the plot.

5. Modify your density plot to change the **LEGEND title** to "Iris Species" instead of the default "Species". Hint: legend titles are part of the _scale_ grammar, so you can find an argument to place in your call to `scale_fill_manual()` that will modify the legend title. Use the help menu!



6. Examine this code and the resulting figure. You will notice it does not work as you might think. Figure out why and fix the problem by playing around with the code. Hint: What colors appear, the ones we specified or the default colors? Are they colors or fills? Is there any _inconsistency_ to the code?
    ```{r, message=FALSE}
    ggplot(iris, aes(x = Sepal.Length)) + 
...
+ What aesthetics (colors, fills, etc) are applied? Are these true aesthetics (mapped from the data), or visual components that are fundamentally not related to the data itself?
  + Colors don't have to be _exact_ matches, but try your best.
...
**You should do these exerises in an R Script which creates and saves each plot to a file. The saved plots should have READABLE aspect ratios.** Your script will be separate from coding in the console, so you will have to load the `ggplot2` library at the very top of the script file.
