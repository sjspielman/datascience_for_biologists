...
Note that these datasets have been loaded for you into the HW3 RStudio Cloud Project. *If there is a problem* such that the datasets get deleted and you need to reload them, copy/paste this code into **the R _Console_**:

```{r, eval=F, echo=T}
source("load_data.R")
```

In theory, you will never need to run this code, but it is an option in case things go wrong.
