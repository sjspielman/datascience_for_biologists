---
title: "Homework #5: Due 2/23/21 as an R Script to Canvas"
...
First, as is templated, you will need to read in all of the datasets and save them to variables, using the `read_csv()` function from the `{readr}` package for this step, and using all your new fancy path knowledge! Then, for each dataset:
...
+ Use `ggplot2` to create a figure *of your choosing and styling* that addresses the question. Each plot should be saved to an output folder called `figures/`. 
  + Plot files should be named as `<dataset>_figure.png` (e.g., `abalone_figure.png`). *Please name your files PRECISELY!*
...
+ Make sure you are commenting out any code that does not specifically contribute to the homework. For example, `View()` should never be in scripts. Similar, while exploring a dataset with functions like `head()`, `names()`, or `str()` is completely and entirely necessary for completing this assignment, the code you use to do so is not specifically part of the plots you make. Therefore, either run those commands directly in Console, or comment them out in your final script.
...
+ `pb` : diastolic blood pressure (units: mm Hg)
...
`conduct` : The conductivity of the urine. Conductivity is proportional to the concentration of charged ions in
solution.
`urea` : The urea concentration in millimoles per liter calcium : The calcium concentration in millimoles per liter
