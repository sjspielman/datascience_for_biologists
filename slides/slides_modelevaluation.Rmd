...
## Cross validation with training and testing sets
...
trained_model <- step( lm(Sepal.Length ~ ., data = training_iris), trace=F   )
```
...
my_iris_model <- function(input_data){
  step(lm(Sepal.Length ~ ., data = input_data), trace =  F)
}
...
iris_kfold %>%
  mutate( model_fit = purrr::map( train, my_iris_model ),
          ...
          test_rsq  = purrr::map2_dbl(model_fit, test, rsquare)) -> result_iris_kfold
```
...
```{r}
# Reminding you of this function we wrote!
my_iris_model <- function(input_data){
  step(lm(Sepal.Length ~ ., data = input_data), trace =  F)
}
...
broom::tidy(fit) # just to see which predictors are there - and it's all!!
...
```{r}
## Predict using a TIBBLE with columns EXACTLY NAMED as MODEL PREDICTORS
