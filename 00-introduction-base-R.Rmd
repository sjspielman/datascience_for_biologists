...
+ Understand R datatypes and their purpose
+ Understand and apply standard R functions
+ Understand the anatomy of a data frame
+ Obtain "summary statistics" from data frames
...
This image presents the **RStudio Cloud** environment (see next section) within a project I have created called "demo". 
...
> Add, subtract, multiply, and divide with operators `+`, `-`, `*`, `/` .<br>
> Exponentiate with either operator `**` or `^`.

#### Do this:

1. What is 780 times 1236.2?
2. What is 1000 divided by 342.321?
...
#### Do this: Redo all your mathematical calculations using VARIABLES!
...
### Functions
We can use pre-built computation methods called "functions" for other operations. Functions have the following format, where the _argument_ is the information we are providing to the function for it to run.
...
#### DO THIS: Perform the following computations using functions
...
|`factor`       | A special type of `character` that indicates CATEGORIES | `"Happy"` and `"Sad"` | `as.factor()`
...
#### DO THIS: Solve the questions below using logical operators. 
> Each answer should end up as TRUE or FALSE, as determined by R - not by you!

1. Is 134 less than 3?
2. Is the number 150 positive or negative? (Hint: compare it to 0)
3. Is the natural log of 134 less than 3?
4. Is the square root of 81 greater than the cube root of 125?
    
    > Hint: Define two variables and use a logical operator to compare them to one another
    
5. Define a character variable consisting of your name, e.g. mine is `name <- "Stephanie"`. Use a logical operator to determine if the _number of characters_ in this overall variable is equal to 7. 
    
    > Hint: To determine the number of characters, use the `nchar()` function. Look it up with `?nchar` if you need help using it!



### Arrays

You will have noticed that all your computations tend to pop up with a `[1]` preceding them in R's output. This is because, in fact, all (ok mostly all) variables are _by default_ **arrays**, and our answers are the first (in these cases only) value in the array. **See [here](https://r4ds.had.co.nz/vectors.html) for a deeper dive into array structures.**

```{r}

## This is actually an array that has _one item_ in it.
x <- 7
...
my_mixedtype_array <- c("one", "one", "two", 3, 5, 8, 13, "twenty one")
length(my_mixedtype_array)

# Combining two arrays will make one BIGGER array, not a nested array
my_array_of_arrays <- c(my_numeric_array,  my_mixedtype_array)
...
## This gives us an array of length 1 whose value will be 3 
my_numeric_array[4]

## Use a _colon_ to specify an inclusive RANGE of indices
...
#### DO THIS: Create, manipulate, and index arrays.
...
1. Define an array variable called `numbers` that contains the range of values 5-15, using a colon `:` to define the variable. Print the length of this array. Perform the following tasks with this array:
    ...
    + We can also use _logical expressions_ to index arrays (example below). Use this approach to obtain all values from `numbers` than are less than or equal to 7.
      ```{r}
    ...
    + Square root all values in each of your two arrays, and save the output arrays respectively as `numbers_sqrt` and `numbers_minus10_sqrt`. Print these variables out. What do you see and why? Take a moment to really understand any unexpected output.
...
2. Define a _logical_ array as `logical_array <- c(TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, TRUE)` (NOTE: you can write `TRUE` or `T` but never `true`, `True`, or `t`. In fact, `t()` is a totally separate function for transposing matrices that we will not be using.)
   ...
   + The function `which()` is sort of the opposite of indexing: It tells you at which index/indices a _logical condition_ is satified. This function takes a single argument which itself is a _logical expression_, as shown in the below example, and returns an array. Use the `which()` function to determine which values in `logical_array` are `TRUE`. 
      ```{r}
...
__This concludes the Introduction to R and RStudio lesson. You should now complete the associated assignment.__
