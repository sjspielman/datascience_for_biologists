...
These exercises use a pre-loaded dataset called `msleep` (sound familar?), which provides different physical and behavioral characteristics of mammals, including how much they sleep.
...
1. Use `mutate()` to create a new column called `class` which literally just contains the string "Mammalia".

> Hint: Use mutate as: `mutate(new-column-name = valuefornewcolumn)`
...
msleep %>%
  # first, add the column class
  mutate(class = "Mammalia") %>%
  # second, only keep the column class
  select(class)
```
...
msleep %>%
  # first, add the column class
  mutate(class = "Mammalia") %>%
  # first, add the columns class and vore
  select(class, vore)
```
...
Practice running code one line at a time below by first writing and running the  `mutate()` code. Make sure there is no error. THEN, tack on the `select()` code to keep only the new "class" variable.
...
5. Use `mutate()` to create a new column called `sleep_awake_ratio` that has the ratio of total time spent awake to total time spent asleep (divide total time asleep by time spent awake).
...
is_it_a_goat <- ifelse(animal == "goat", "goat", "not a goat")

# FIRST, think about what value you expect `is_it_a_goat` to be. Then, print it in the space below to see:
```

`dplyr` actually has *its own* version of this function called `if_else()` (it has an underscore). This version of the function is technically faster and "safer" to use in your `dplyr` code, but either `ifelse()` or `if_else()` will be fine for the purposes of our class, but we'll use `if_else()` to get in the habit!
...
>
> Hint 2: Again, it's helpful to use `select()` to make sure you did it all correctly!
...
>
> Hint 2: For more coding style, use a variable for the value of 100 instead of directly including it in the `mutate()` code.
...
8. Create a new column in `msleep` called `needs_more_caffeine` where mammals who are awake (`awake`) more than 16 hours a day have the value "definitely" and other mammals have "nope". Make sure to define the value 16 separately to *avoid hardcoding!!*
