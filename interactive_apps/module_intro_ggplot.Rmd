...
7. You'll notice the legend title came out a little like garbage in the last plot. Let's learn how to set our own legend titles. Legends will appear when you use an aesthetic mapping - sometimes a `color`, sometimes a `fill`, or even `shape`, `size`, and more. You can customize the legend with the function `scale_AES_manual()`, where you replace `AES` with the type of aesthetic. In this plot, we are mapping Age to *color*, so we use the function `scale_color_manual()`. We can use the argument *name* to change the legend title, as in: `scale_color_manual(name = "Legend title")`.
...
ggplot(sparrows) + 
  aes(x = Wingspread, 
      ...
      color = fct_relevel(Age, c("Juvenile", "Young", "Adult") )) +  
  geom_point() + 
  scale_color_manual(name = "Sparrow age")
```
