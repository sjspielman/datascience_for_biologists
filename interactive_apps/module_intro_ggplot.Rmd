...
You will see the dataset has three categorical variables that R refers to as "factors." Each of these variables has two *levels*, which means "categories in the categorical variable." In this dataset, for example, the variable `Age` has two levels: `Adult` and `Young`. You will see the remaining 9 variables are all numeric continuous. 
...
```{r summary-sparrows, exercise=TRUE}
...
```{r summary-column, exercise=TRUE}
summary(sparrows$Wingspread)
```

```{r summary-column-solution, exercise=TRUE}
...
> Explore and learn more about the different types of plots [here](https://rowanbiosci.shinyapps.io/types_of_plots/)

Using `geom_point()` to make scatterplots
...
### **ALERT!** An *extremely common* bug/typo you can now look forward to doing for the rest of your life is accidentally placing a column *outside* of `aes()`. Here's an example of that bug. Run the code and interpret the error message (see hint below!), and then fix the code to actually show points colored based on `Survival`.
...
## NO - this is a bug in plotting code because you cannot start a line with + (or ,)
ggplot(sparrows) + 
  aes(x = Wingspread, y = Femur_Length) 
  + geom_point()
```
...
6. The code below makes a scatterplot of femur length across wingspread with points colored by _Age_. You'll notice in the legend that "Young" comes before "Adult" - this is because R has, by default, assumed the levels in Age (Adult and Young) should be ordered alphabetically. Run the code to see this in action. 
...
What if we wanted the order to be different and show "Young" before "Adult" along the x-axis? There are several strategies to accomplish this task. We will learn how to do this using a function from the [`forcats`](https://forcats.tidyverse.org/reference/) library, a `tidyverse` library that helps change the order of a factor variable. The package name involves several puns, and the library has been pre-loaded for you in these exercises. 
...
# First argument is the column we want to reorder, Age. This is a column so NO QUOTES!
# Second argument is an *array* of the new order. These are strings, not columns, so YES QUOTES!
fct_relevel(Age, c("Adult", "Young"))
...
> When you change the factor level like this, the label comes out like garbage.... We will learn how to change legend titles soon!

```{r scatter_6_demo2, exercise = TRUE}
# Notice how I space out the aes() arguments on separate lines. MUCH easier to read!
ggplot(sparrows) + 
  aes(x = Wingspread, 
      y = Femur_Length, 
      # instead of Age, give the whole statement:
      color = fct_relevel(Age, c("Young", "Adult") )) +  
  geom_point()
...
7. Using the code above to practice and learn how to manipulate factors, make a scatterplot below of femur length across wingspread, with points colored by `Sex`. First make the plot without considering the order of factors, and then make it again with the order of `Sex` levels reversed using the skills from the previous example.
...
1. Make a simple histogram of sparrow Wingspread using `geom_histogram()`. Remember: You only should specify the x-axis; `ggplot2` will do the y-axis for you since the y-axis is a *statistical transformation* (count).
...
> Don't forget [all the R colors you can use!!](https://github.com/sjspielman/datascience_for_biologists/raw/master/spring2020/rmd_lessons/static/Rcolor.pdf)!
...
6. Don't forget your factors! Remind yourself that you know how to use the `fct_relevel()` function by reordering the previous plot to show Young before Adult on the x-axis.
...
```{r bar_6-solution}
ggplot(sparrows, aes(x = fct_relevel(Age, c("Young", "Adult")), 
                     fill = Survival)) + 
  geom_bar(position = position_dodge()) +
  labs(x = "Age")
```
...
7. For fun, make it a *horizontal* barplot!! Simply change the `x` mapping to `y` instead, and don't provide an `x` aesthetic. 
...
2. Shapes 21-25 are actually a little different from other shapes - they can be colored *and filled*. Again, for 2D shapes, color means *outline* and fill means *fill*. Re-make the scatterplot but specify shape = 21, and specify a color and fill of your choosing to `geom_point()` to see this in action.
...
6. Let's explore the `geom_smooth()` function further by modifying the plot you made for the previous question in the following ways:
  ...
  + Explore what adding `color` and `fill` arguments to geom_smooth does. Use these arguments in the same way you'd provide "just a color/fill" to `geom_point()` (these are not mapped aesthetics). One thing you will discover: `fill` doesn't really do much if `se=FALSE`. Understand why this is by trying lots of code!
