...
7. You'll notice the legend title came out a little like garbage in the last plot. Let's learn how to set our own legend titles. Legends will appear when you use an aesthetic mapping - sometimes a `color`, sometimes a `fill`, or even `shape`, `size`, and more. You can customize the legend with the function `scale_<aesthetic>_<discrete/continuous>()`, where you replace `<aesthetic>` with the type of aesthetic. Further, depending on *the type of variable you are mapping to*, you specify the final part of the function as either `discrete` or `continuous`. 

  In this plot, we are mapping Age to *color*, and Age is discrete. so we use the function `scale_color_discrete()`. We can use the argument *name* to change the legend title, as in: `scale_color_discrete(name = "Legend title")`.

Re-make the plot below with the legend title "Sparrow age":

> Hint: Like any `ggplot2()` function, we simply add it to the plot
...
ggplot(sparrows) + 
  aes(x = Wingspread, 
      ...
      color = fct_relevel(Age, c("Juvenile", "Young", "Adult") )) +  
  geom_point() + 
  scale_color_discrete(name = "Sparrow age")
```


8. Using the code above to practice and learn how to manipulate factors for plotting, make a scatterplot below of femur length across wingspread, with points colored by `Sex`. First make the plot without considering the order of factors, and then make it again with the order of `Sex` levels reversed using the skills from the previous example. Make sure you produce a clean legend title! Again, we are mapping to color and Sex is discrete, so we use `scale_color_discrete()`.
