...
#### **See [here](http://htmlpreview.github.io/?https://raw.githubusercontent.com/sjspielman/datascience_for_biologists/master/docs/tutorials/introduction_to_R.html) for the "Introduction to R tutorial"**
...
## Set 4: Working with logical statements
...
### Working with variables and logical statements
...
1. Is 12 greater than -4?
  
```{r logical_q11, exercise = TRUE}
...
1. Is the square root of 36 equal to 5? *Make sure to engage with this code and not just look at it!*
...
2. The function `nchar()` asks how many characters are in a string (how long is a string?). See this function in action below:

```{r practice_nchar, exercise=TRUE}
nchar("hello")
...
4. Let's combine our skills! Define a variable called `x` whose value is 7 (no quotes!). Then, determine its type with `class()`, but **save this output** to a new variable called `type_x`. Finally, use logical operators with this variable to ask: Is the type of the variable equal to "numeric"? Then ask, is the type of the variable equal to "character"?
...
5. Is the natural log of 10 less than the natural log of 100 *and* is the natural log of 10 equal to 1.5?
  
  > Hint: the `log()` function calculates the natural log by default

```{r logical_fxn_5, exercise = TRUE}

```
```{r logical_fxn_5-solution}
...
5. Modify the code from the previous question to use a *logical operator* to ask whether the length of your array equals 5. If done correctly, your answer should be `TRUE`.
...
```{r array_q5-solution}
some_numbers <- c(10, 11, 2, 19, 6)
length(some_numbers) == 5
```

6. Create another array with FOUR values, and this time make sure all the values are *strings* (each value should be surrounded by quotes). Use the function `length()` to ask how long the array is. *Then*, ask if the length of the array is equal to 7.
...
> Hint 2: The output for your logical comparison should be `FALSE` if done correctly.
...
### Arrays and the new logical operator `%in%`
...
```{r in_example, exercise=TRUE, exercise.lines = 15}
# Define array
...
**Something else** you'll notice about the above code is that it prints THREE different outputs, in order of the code. It can very be challenging to see exactly which piece of code produced which output, especially when our code gets longer and more involved. **Printing messages along the with the `print()` function to guide you when reading output will be you SAVIOR!! IT'S SO IMPORTANT!** In fact, if you only learn one thing this semester, it's to PRINT PRINT PRINT!


```{r in_example_print, exercise=TRUE, exercise.lines = 15}
# Define array
rainbow <- c("red", "orange", "yellow", "green", "blue", "indigo", "violet")

# Will return TRUE
print("Is red in rainbow?")
"red" %in% rainbow

# Will return FALSE
print("Is black in rainbow?")
"black" %in% rainbow

# This is _not_ The Way 
# This makes a logical array, not a single TRUE/FALSE
...
3. Using your new skills, calculate the _median_ hours of REM sleep (columns `sleep_rem`) these mammals get. The output will not look like you expect - instead of a number, you get the output "NA". To start to understand why, view the full contents of this column (i.e. `msleep$sleep_rem`). You will see some values "NA", meaning there is *missing data*. Missing data is very common - real life science (especially biology!) is tricky, and sometimes we can't gather as much data as we want. 
...
## Set 7: Conditional variable definition
...
2. Define a variable based on whether "Cheetah" is an animal in `msleep` (use the `name`) column. If the condition is `TRUE`, the variable value should be "present". If the condition is `FALSE`, the variable value should be "absent".

> Hint: Don't forget the `%in%` operator!
...
3. Define a variable based on whether the *maximum body weight* in `msleep` is greater than 2000. If the condition is `TRUE`, the variable value should be 1 (the number 1, no quotes!). If the condition is `FALSE`, the variable value should be 0.
...
4. Define a variable based on whether number of rows in `msleep` equals 80. If the condition is `TRUE`, the variable value should be 10. If the condition is `FALSE`, the variable value should be -10.
